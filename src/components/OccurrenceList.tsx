...
export const OccurrenceList: React.FC<OccurrenceListProps> = ({
  ...
  // Filter logic
  const sortedOcorrencias = [...ocorrencias].sort((first, second) => String(second.dataHora || '').localeCompare(String(first.dataHora || '')));
  const filtered = sortedOcorrencias.filter((occ) => {
